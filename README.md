...
| :------------------ | :-------------------------- | :------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | :--------------------------------------------------------------------------- |
| `map from to`       | Manual Field Mapping        | Copygen uses its [automatcher](#automatch) by default. <br /> Override this using `map` to set fields that will be mapped to and from each other. <br /> Regex is supported for from-fields. | `map .* package.Type.Field` <br /> `map models.Account.ID domain.Account.ID` |
...
_Use of Go Modules in templates are unsupported at the current time._
...
Copygen provides two ways to configure fields: **Manually** and using the **Automatcher**. Matching is specified in a `.go` file _(which functions as a schema in relation to other generators)_. Support for tags is coming soon.
