...
var (
	...
	//
	// setupPkgPath is used to remove package references from types that are
...
func Parse(gen *models.Generator) error {
	...
	if err := printer.Fprint(buf, p.Config.Fileset, p.Config.SetupFile); err != nil {
		return fmt.Errorf("an error occurred writing the code that are kept after generation\n%w", err)
	}
	gen.Keep = buf.Bytes()
